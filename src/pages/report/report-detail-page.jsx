  ...
  const handleDownloadExcelEachFeetype = async (feeTypeName, feeTypeId) => {
    try {
      const response = await apiClient.get(
        `/report/fee-type-report-excel/${id}/${feeTypeId}/`,
        {
          headers: {
            Authorization: `Bearer ${getToken()}`,
          },
    ...
    <>
      ...
      <BreadcrumbComponent
        customItems={[
          ...
          { label: `Report - ${standard === "13" ? "Balvatika" : standard}` },
        ]}
      />
      {/* PAth */}
      <div className="container mx-auto px-4 pb-8">
        <h1 className="uppercase md:text-2xl text-sm font-bold mb-4">
          Fee Report - Standard / {standard === "13" ? "Balvatika" : standard}
        </h1>
        <div className="grid grid-cols-1 sm:grid-cols-3 lg:flex lg:flex-wrap gap-4 mt-8 mb-6">
          ...
          {fee_types?.map((feeType) => (
            <div key={feeType.id} className="w-full sm:w-auto">
              <Button
                onClick={() =>
                  handleDownloadExcelEachFeetype(feeType.name, feeType.id)
                }
                className="w-full"
                variant="outline"
              >
